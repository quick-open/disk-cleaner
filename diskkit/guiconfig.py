r"""Tiny JSON-backed config for the Disk Cleaner GUI.

Stores just the chosen theme ("light"/"dark") and a short list of recently used
folders (Large Files / Disk Usage pickers).  On Windows the file lives at
``%LOCALAPPDATA%\DiskCleaner\config.json``; elsewhere it falls back to
``~/.diskcleaner/config.json``.  Every function is defensive -- a corrupt or
...
def config_dir():
    ...
    ``%LOCALAPPDATA%\DiskCleaner`` on Windows, ``~/.diskcleaner`` otherwise.
    """
